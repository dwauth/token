...
var dwidAccess = require('@dwauth/access')
var dwidAuth = require('@dwauth/auth')
var basic = require('dwid-auth/basic')
var memdb = require('memdb')

var dwidToken = require('./index')
...
test('sign and verify a token with secret', function (t) {
  var db = memdb()
  var tokens = dwidToken(db, { secret: 'not a secret' })
...
test('sign and verify a token with keypair', function (t) {
  var db = memdb()
  var tokens = dwidToken(db, { publicKey: publicKey, privateKey: privateKey, algorithm: 'ES512' })
...
test('use with dwid-auth and dwid-access', function (t) {
  var db = memdb()
  var tokens = dwidToken(db, { secret: 'not a secret' })
  var access = dwidAccess(db)
  var auth = dwidAuth(db, {
